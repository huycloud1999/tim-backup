...
import { getAllNews, getOngttNews, getRelatedNews } from "../store/action/new";
    ...
    <>
      ...
      {props.NewDetail && (
        <ArticleJsonLd
          url={link}
          title={props.NewDetail.title}
          images={[props.NewDetail.featuredImage.node.sourceUrl]}
      ...
      <PostGrid
        heading={<div>related News & Insights</div>}
        data={props.AllNew}
      />
    </>
  );
...
export async function getStaticPaths() {
  ...
  const paths = res?.data?.data?.posts?.nodes?.map((data) => ({
