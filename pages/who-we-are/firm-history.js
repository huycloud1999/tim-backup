...
export default function FirmHistory(props) {
  ...
  function onHandleSelect(index) {
    ...
    gsap.to(tabList.current, {
      duration: 1,
      scrollTo: { x: elementToScroll, offsetX: (tabList.current.clientWidth / display.current) * 1 },
      ease: "power2.out",
    });
    var greenLine = document.querySelector(".progress");
    var boxGreen = document.querySelector(".box_history");

    greenLine.style.width = ((index + 1) / length * 100 - ((100 / display.current / 2) / (length / display.current))) + "%";
    boxGreen.style.left = ((index + 1) / length) * 100 - ((100 / display.current / 2) / (length / display.current)) + "%";
  ...
  return (
    <>
      ...
      />
      <SmallTitle title={firmhistory.subTextSection.title} className={"certoffering firmHistory_title"}>
        {firmhistory.subTextSection.description}
      </SmallTitle>
      <div className="firm_history">
        <div className="container">
          <div className="firm_history_text"></div>
          <div className="firm_history_tab" onMouseOver={stopSlide} onMouseOut={continueSlide}>
            <Tabs
                      ...
                      <div className="tab">
                        <div className="tab_panel">
                          <div className="image-container panel_img ">
                            <Image src={value?.img} alt="image model" layout="fill" className="image-item" />
                          </div>
