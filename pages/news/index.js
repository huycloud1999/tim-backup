...
import { getFilterNews, getAllCat, getAllNews, getRelatedNews } from "../../store/action/new";
...
export default function NewsPage(props) {
  ...
  if (pageCount > 1) {
    ...
    for (let index = 0; index < pageCount; index++) {
      ...
      if ((index > 3 && index <pageCount-3 && index!=page-1) && (index< page-3  || index> page +2) ){
        ...
        pagination.push(<div key={index} className={"pagination-item" + active} onClick={()=>handlePagination(index)}>{index+1}</div>)
      }
  ...
  useEffect(()=>{
    ...
    async function getListNews() {
      ...
      const res = await getFilterNews(router.query.category|| null, router.query.year || null, postsPerPage,(size-1) * postsPerPage || 0, 1000,'');
      ...
      var indexCat = dataCatClone.indexOf(parseInt(router.query.category || null))
      var indexYear = yearClone.indexOf(parseInt(router.query.year || null))
      
   
      
      setYear((optionsYear[indexYear] && optionsYear[indexYear].value) || optionsYear[0].value )
